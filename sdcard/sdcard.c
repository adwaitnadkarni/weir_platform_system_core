...
struct node {
    ...
    //WEIR: Is this a weir directory, or a subdirectory of a weir directory?
    int weir_dir;
};
...
static void derive_permissions_locked(struct fuse* fuse, struct node *parent,
    ...
    //WEIR: TODO: Find a better comparison. May be compare the entire path, or
    //the path up to a point?
    node->weir_dir = (parent->weir_dir==1)?parent->weir_dir:0;
    if(node->weir_dir == 0 && (strstr(node->name,"wdir") != NULL || strstr(parent->name,"wdir") != NULL)){
	node->weir_dir=1;
    }
    ...
    case PERM_ROOT:
        ...
        if (!strcasecmp(node->name, "Android")) {
            ...
            node->mode = 0771;
        } else if(!strcasecmp(node->name, "weir")){
	    node->perm=PERM_WEIR;
	    node->mode=0333;
...
static bool check_caller_access_to_name(struct fuse* fuse,
	...
	//WEIR
	/*char buf[1000]; int bufsize=1000;
	if (node && get_node_path_locked(node, buf, bufsize)!=NULL)
	if(strcasecmp(parent_node->name, "wdir_0")){
	    return true;
	}*/
        return has_rw;
    }
...
//    case FUSE_SETXATTR:
    case FUSE_SETXATTR: {
	    const struct fuse_setxattr_in *req = data;
	    char buffer[PATH_MAX], value[XATTR_SIZE_MAX], name[XATTR_NAME_MAX];
	    int res;
    
	    get_node_path_locked(lookup_node_by_id_locked(fuse, hdr->nodeid), buffer, PATH_MAX);
    
	    strncpy(name, ((char*) data) + sizeof(*req), XATTR_NAME_MAX);
	    memcpy(value, ((char*) data) + sizeof(*req) + strlen(name) + 1, req->size);
    
	    res = setxattr(buffer, name, value, req->size, 0);
    
	    fuse_status(fuse, hdr->unique, res ? -errno : 0);
	    return 0;
	}
    case FUSE_GETXATTR: {
            const struct fuse_getxattr_in *req = data;
            char buffer[PATH_MAX], value[XATTR_SIZE_MAX], name[XATTR_NAME_MAX];
            int res;
	    ...
	    strncpy(name, ((char*) data) + sizeof(*req), XATTR_NAME_MAX);
	    res = getxattr(buffer, name, value, req->size);
	    if (res < 0) {
